...
import { useTheme } from '../context/ThemeContext'; // Local ThemeContext
...
// Create a GoogleIcon component directly in the file
const GoogleIcon = () => (
...
const handleMFAChallenge = async (resolver: any, verificationCode: string) => {
  const phoneAuthCredential = PhoneAuthProvider.credential(
    resolver.hints[0].sessionInfo,
    verificationCode
  );
  return signInWithCredential(resolver.auth, phoneAuthCredential);
};

// SMS helpers
const validatePhoneNumber = (phoneNumber: string): boolean => {
  // Simple validation for Indian phone numbers
  return /^\+91[1-9]\d{9}$/.test(phoneNumber);
};

const formatPhoneNumber = (phoneNumber: string): string => {
  // Format phone number (add +91 if needed)
  if (phoneNumber.startsWith('+91')) return phoneNumber;
  return `+91${phoneNumber.replace(/^0/, '')}`;
};
...
const handleEmailSignIn = async (email: string, password: string) => {
  // Special handling for admin account to avoid Firebase error
  if (email === 'adm.railmadad@gmail.com' && password === 'admin@2025') {
    localStorage.setItem('isAuthenticated', 'true');
    localStorage.setItem('userRole', 'admin');
    localStorage.setItem('adminToken', 'admin-token');
    return { success: true, redirect: '/dashboard', isAdmin: true };
  }

  // For non-admin accounts, use Firebase auth
  const auth = getAuth();
  try {
...
const Login: React.FC = () => {
  ...
  const handleEmailPasswordSignIn = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // Check for admin login first to bypass Firebase
      if (email === 'adm.railmadad@gmail.com' && password === 'admin@2025') {
        localStorage.setItem('isAuthenticated', 'true');
        localStorage.setItem('userRole', 'admin');
        localStorage.setItem('adminToken', 'admin-token');
      ...
      setMessageType(showMessage(errorMessage, setError, 'error'));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Regular user authentication through Firebase
